...
blocks.beh <- expand.grid(group = countries, het.beh=T, het.beh.sd=seq(0,3, by = .1), het.beh.cor=seq(0,1, by = .05), bmb.sc = 1)
...
blocks.beh <- blocks.beh[with(blocks.beh, order(group, het.beh.sd, het.beh.cor, bmb.sc)),]

blocks.gen <- expand.grid(group = countries, het.gen=T, het.gen.sd=seq(0,3, by = .1), het.gen.cor=seq(0,1, by = .05), bmb.sc = 1)
...
blocks.gen <- blocks.gen[with(blocks.gen, order(group, het.beh.sd, het.beh.cor, bmb.sc)),]
...
outdir <- file.path('results','AssortHetHeatMap')
...
load(file=file.path(outdir, 'JobsToDo.Rdata'))
