library(data.table); library(graphics); library(abind); library(mvtnorm); library(parallel) # load necessary libraries
...
######################################################################
ts.fxn <- function(dat, nc = 12, verbose = T, vfreq = 500, return.ts = F, browse = F)
    {
        if(browse)  browser()
            ...
            if(dat$dage[cc] < dat$tend[cc]) # if they aged out before interview time or AIDS related death
              {
                ...
                ts[1:1332 > dat$dage[cc],cc] <- paste('a.',ts[1:1332 == dat$dage[cc],cc], sep = '')
                if(dat$dmage[cc]<=dat$dfage[cc]) # if male is first
                  {
                    ts[1:1332 > dat$dage[cc],cc] <- paste(ts[1:1332 > dat$dage[cc],cc], '.m', sep = '') # add m
                  }else{                # otherwise
                    ts[1:1332 > dat$dage[cc],cc] <- paste(ts[1:1332 > dat$dage[cc],cc], '.f', sep = '') # add f
                  }
              }
          }
      }
